...
const Adminpage = () => {
  ...
  const loadStudentDetail = async (studentId, preservedStudent = null) => {
    if (!studentId) {
      return
    }

    if (preservedStudent) {
      setSelectedStudent(preservedStudent)
    }
    ...
    try {
      const data = await apiRequest(`/api/admin/students/${studentId}`)
  ...
  useEffect(() => {
    ...
    const refreshStudentLists = () => {
      const currentQuery = studentQueryRef.current || studentQuery
      loadStudents({
    ...
    const handleProgressUpdate = (event) => {
      ...
      if (detail.student) {
        patchStudentState(studentId, detail.student)
      }

      if (selectedStudent && getStudentRowId(selectedStudent) === studentId) {
        loadStudentDetail(studentId, selectedStudent)
      }
    }
    ...
    const handleClassFeedUpdate = (event) => {
      const detail = event?.detail || {}
      const updatedClassId = String(detail.classId || '')

      if (activeTab !== 'class-board') {
        return
      }

      if (!updatedClassId || updatedClassId === String(selectedClassFeedId || '')) {
        loadClassFeed(selectedClassFeedId)
      }
    }

    window.addEventListener('innovative-science-progress-updated', handleProgressUpdate)
    window.addEventListener('innovative-science-class-roster-updated', handleClassUpdate)
    window.addEventListener('innovative-science-class-feed-updated', handleClassFeedUpdate)

    return () => {
      window.removeEventListener('innovative-science-progress-updated', handleProgressUpdate)
      window.removeEventListener('innovative-science-class-roster-updated', handleClassUpdate)
      window.removeEventListener('innovative-science-class-feed-updated', handleClassFeedUpdate)
    }
  }, [selectedStudent, studentQuery, activeTab, selectedClassFeedId])
  ...
  const openStudentDetail = async (student) => {
    ...
    await loadStudentDetail(student.id, student)
  }
                          ...
                          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            >
                              ...
                              {student.className ? (
                                ...
                                <p className="mt-2 text-xs font-black uppercase tracking-[0.22em] text-slate-300">
                                  No class assigned
                                </p>
                              )}
                            </button>
                            ...
                            <div className="grid gap-2 sm:min-w-[250px]">
                              <div className="grid grid-cols-2 gap-2">
                                <div className="rounded-2xl bg-white p-3 text-center">
                                  ...
                                  <p className="mt-2 text-xl font-black text-slate-950">{Number(student.totalBrainCells || 0)}</p>
                                </div>
                                <div className="rounded-2xl bg-white p-3 text-center">
            ...
            {detailLoading ? (
              ...
              <>
                <div className="mt-6 grid gap-3 sm:grid-cols-3">
                  <Stat label="Brain cells" value={Number(studentDetail.progress?.totalBrainCells ?? studentDetail.user.totalBrainCells ?? 0)} />
                ...
                <div className="mt-6 grid gap-4 lg:grid-cols-2">
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    {studentDetail.user.className ? (
                      <p className="mt-2 text-lg font-bold text-slate-700">{studentDetail.user.className}</p>
                    ) : (
                      <p className="mt-2 text-lg font-bold text-slate-400">No class assigned</p>
                    )}
                  </div>
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">

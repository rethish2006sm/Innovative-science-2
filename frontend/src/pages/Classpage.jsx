...
export default function Classpage() {
  ...
  useEffect(() => {
    if (!auth?.user?.isAdmin && auth?.user?.classId) {
      if (String(auth.user.classId) !== String(classId)) {
        navigate(`/class/${auth.user.classId}`, { replace: true });
      }
      return;
    }

    if (!auth?.user?.isAdmin && !auth?.user?.classId && classId) {
      navigate("/", { replace: true });
    }
  }, [auth?.user?.classId, auth?.user?.isAdmin, classId, navigate]);

  useEffect(() => {
    const refreshFeed = (event) => {
      const detailClassId = String(event?.detail?.classId || "");

      if (!detailClassId || detailClassId === String(classId)) {
        setReloadTick((current) => current + 1);
      }
    };

    window.addEventListener("innovative-science-class-feed-updated", refreshFeed);

    return () => {
      window.removeEventListener("innovative-science-class-feed-updated", refreshFeed);
    };
  }, [classId]);
